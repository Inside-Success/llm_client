...
| LLM-RUNTIME-006 | Structured callers could not require native JSON schema; provider schema rejection silently changed execution to Instructor, whose internal retries are separate from `RetryPolicy`. | High | Strict routing is implemented. Independent reviews rejected exact-replay repairs `9016721` for envelope integrity and `f63788b` for a real timeout-ban producer/consumer mismatch plus lossy value coercions mislabeled exact. | Require every reproduced attack, real four-path capture-to-replay tests, full tests, and fresh exact-commit independent acceptance before Plan 0141 binds it. |
