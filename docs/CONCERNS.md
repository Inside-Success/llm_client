...
| LLM-VERIFY-007 | The non-E2E suite appeared stalled because CLI smoke launches 15 cold Python imports and a quota test inherited a real cross-process provider cooldown before reaching its mock. | Medium | Verbose execution identified both waits; the quota-registration test now neutralizes pre-existing cooldown state while still exercising registration. | Rerun the mandatory Plan 99 completion helper; separately consider current-test/timeout reporting for future diagnosis. |
