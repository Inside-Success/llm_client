...
Verification context: provider-independent structured execution, exact effective-policy snapshot/replay, structured-attempt evidence, strict selected-attempt and opt-in raw-content reads, agent-invocable raw retention cleanup, and fail-loud trace-joinable tool-call lifecycle persistence are implemented once in the shared runtime; focused controls pass.
