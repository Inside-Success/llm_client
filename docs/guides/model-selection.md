...
| `openrouter/openai/gpt-5.6-luna` (medium) | explicit low-latency planner candidate | **Native JSON schema (changed 2026-08-20; was Instructor default)** | The 2026-07-29 Process Tracing probe found no endpoint accepting *that* schema's structured parameter profile, and Luna was pinned to Instructor on 2026-08-07. Re-probed 2026-08-20 against the Inside Success bounded-extraction schema: OpenRouter accepted the native request (`response_format.json_schema`, `strict=true`, `additionalProperties=false`, `finish_reason=stop`, valid JSON). The Instructor transport was unsafe here — with the schema unenforced the model appended array items to the 65536-token ceiling and returned JSON truncated mid-string, at ~14x the cost of a healthy call. Per-schema variation is real; re-probe before assuming either result transfers. |
