...
| GPT-5.5, direct or OpenRouter | historical only | **Hard-blocked before dispatch** | Plan 110 retires the whole GPT-5.5 family from new calls. The retained direct strict-schema receipt remains historical transport evidence; use GPT-5.6 Sol or Terra for new work. |
| GPT-5.4 Mini, direct or OpenRouter | historical only | **Hard-blocked before dispatch** | Plan 110 retires this family from new calls and fallbacks. Use Luna only for a bounded low-cost route with task evidence, or Terra where stronger judgment is needed. |
