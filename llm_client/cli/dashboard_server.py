...
def _page(data: dict[str, Any]) -> str:
    ...
    return "<!doctype html><meta http-equiv='refresh' content='30'><title>LLM Cost Dashboard</title><style>body{background:#10131a;color:#e8edf5;font:16px system-ui;margin:3rem;max-width:1100px}main{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1rem}section{background:#191f2b;border:1px solid #344054;border-radius:12px;padding:1.25rem}strong{font-size:2rem;color:#82d6ff}.alert{color:#ffb4a9;font-weight:bold}small,a{color:#aab4c4}.chart{display:block;width:100%;height:240px;background:#191f2b;border-radius:12px}.bar{fill:#82d6ff}.label{fill:#aab4c4;font-size:11px}</style><h1>LLM Cost Dashboard</h1><small>Local ledger · refreshes every 30 seconds · <a href='/?granularity=hour'>hourly</a> · <a href='/?granularity=day'>daily</a> · <a href='/api/dashboard'>JSON</a></small><main>" + "".join(cards) + "</main><h2>Spend over time (" + data["granularity"] + ")</h2><svg id='series' class='chart' viewBox='0 0 1000 240' preserveAspectRatio='none'></svg><h2>Project breakdown</h2><svg id='projects' class='chart' viewBox='0 0 1000 240' preserveAspectRatio='none'></svg><h2>Model breakdown</h2><svg id='models' class='chart' viewBox='0 0 1000 240' preserveAspectRatio='none'></svg><script>const d=" + json.dumps(data) + ";function bars(id,a){let s=document.getElementById(id),w=1000,h=240,m=Math.max(...a.map(x=>x[1]),.01);a.forEach((x,i)=>{let bw=w/a.length*.72,bh=x[1]/m*(h-35),px=i*w/a.length+(w/a.length-bw)/2;s.innerHTML+=`<rect class=bar x='${px}' y='${h-22-bh}' width='${bw}' height='${bh}'/><text class=label x='${px}' y='${h-6}'>${x[0].slice(-8)}</text>`})}bars('series',d.series);bars('projects',d.projects);bars('models',d.models)</script>"
