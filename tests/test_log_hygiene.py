...
from llm_client.execution.timeout_policy import default_timeout_for_caller
...
class TestTimeoutDisabledSpam:
    ...
    def test_allow_fills_default_and_logs_nothing(
        ...
        is filled and forwarded, no TIMEOUT_DISABLED records.

        Reads the default from timeout_policy rather than pinning a number: it
        was 180s when this test was written (2026-07-16) and is 60s now, and
        the behavior under test is that the library default is filled, not
        which number it is.
        """
        ...
        assert _timeout_disabled_records(caplog) == []
        assert mock_completion.call_args.kwargs["timeout"] == default_timeout_for_caller(
...
class TestProviderFinishReasonMasking:
    ...
    def test_provider_error_reports_provider_delay_source(
        ...
        from llm_client.execution.retry import (
            RetryPolicy,
